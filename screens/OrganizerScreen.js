...
export default class BookRequestScreen extends Component {
  ...
  addRequest = async (bookName, reasonToRequest) => {
    ...
    db.collection("requested_programs").add({
      user_id: userId,
      book_name: bookName,
    ...
    return Alert.alert("Participation Requested Successfully");
  };

  receivedBooks = (bookName) => {
    var userId = this.state.userId;
    var requestId = this.state.requestId;
    db.collection("participation").add({
      user_id: userId,
      book_name: bookName,
      request_id: requestId,
      bookStatus: "participated",
    });
  };
  ...
  render() {
    if (this.state.IsBookRequestActive === true) {
      return (
        <View style={{ flex: 1}}>
          ...
          >
            <MyHeader title="Participation Status" navigation={this.props.navigation} />
          </View>
          <View
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={{ flex: 0.1 }}>
          <MyHeader title="Apply for program" navigation={this.props.navigation} />
        </View>
        <View style={{ flex: 0.9 }}>
          <Input
          ...
          {this.state.showFlatlist ? (
            ...
            <View style={{ alignItems: "center" }}>
              <Input
                ...
                numberOfLines={8}
                label={"Reason"}
                placeholder={"Why do you want to participate in this program(optional)"}
                onChangeText={(text) => {
                  this.setState({
